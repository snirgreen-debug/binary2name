...
def get_values_from_arg_old(kind_cat: Tuple[str, str]) -> List[str]:
...
### ITTAY AND ITAMAR CODE ###
def get_values_from_arg(argument: Tuple[str, str]) -> List[str]:
    arg_type, arg = argument

    if arg_type == CONST_TYPE:
        return [arg]
    else:
        return [arg_type]

### END OF ITTAY AND ITAMAR CODE ###
...
if __name__ == '__main__':
    ...
    for kind in kinds:
        print(get_values_from_arg_old(kind))